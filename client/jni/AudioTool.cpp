 ...
 void AudioTool::float2short(float *floatbuffer, short *shortbuffer, unsigned int length)
{
 ...
 void AudioTool::short2float(short *shortBuffer, float *floatBuffer, unsigned int length)
 {
	 for(int i=0;i<length;i++)
 ...
 float AudioTool::getSampleByFloatIndex(float *buffer, int bufferSampleLength, double sampleIndex, bool left)
{
 ...
 int AudioTool::getMonoSampleByFloatIndex2(float *buffer, int bufferSampleLength, double sampleIndex, float *value)
 {
 	int prevSampleIndex = sampleIndex;
     int nextSampleIndex = prevSampleIndex + 1;
     ...
     if(prevSampleIndex >= bufferSampleLength-1)
     {
    	 *value = buffer[bufferSampleLength-1];
         return 0;
     }
     */
     if(nextSampleIndex >= bufferSampleLength)
    	 nextSampleIndex = 0;

     float prevValue = buffer[prevSampleIndex];

     float nextValue = buffer[nextSampleIndex];
...
int AudioTool::getSampleByFloatIndex2(float *buffer, int bufferSampleLength, double sampleIndex, float *leftValue, float *rightValue)
{
    ...
    if(prevSampleIndex >= bufferSampleLength - 1)
    {
    	*leftValue = buffer[(bufferSampleLength-1)*2];
        *rightValue = buffer[(bufferSampleLength-1)*2+1];
        return 0;
    }
 ...
 long AudioTool::getSystemCurrentTime()
{
 ...
 long AudioTool::getMicroCurrentTime()
 {
	 timespec ts;
	 ...
	 return (sec % INT_MAX) * 1000000 + nano / 1000;
 }


int AudioTool::unwrapPhase(float *phaseArray, int length)
{
	float savedPhase = phaseArray[0];
	for(int i=1;i<length;i++)
	{
		float wrappedDeltaPhase = wrapPhase(phaseArray[i] - savedPhase);
		savedPhase = phaseArray[i];
		phaseArray[i] = phaseArray[i-1] + wrappedDeltaPhase;
	}
	return 0;
}
